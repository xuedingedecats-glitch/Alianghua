...
def opening_check_payload(force: bool = False) -> Dict[str, Any]:
    ...
    common={"source_date":source_date,"schedule":OPENING_SCHEDULE,"run_state":public_run_state(OPENING_RUN_STATE, OPENING_LOCK),"limit":OPENING_CHECK_LIMIT,"watchlist":watch,"monitor_mode":mode,"history":opening_history_payload()}
...
def opening_page_html(payload: Dict[str, Any]) -> str:
    ...
    watch_script = (
        "<script>const openWatch=" + script_json(watch)
...
function riskSettings(){return {capital:riskNumber('riskCapital',100000),riskPct:riskNumber('riskPct',0.5),maxPosPct:riskNumber('maxPosPct',20)}}
...
function loadRiskSettings(){try{const s=JSON.parse(sessionStorage.getItem('opening_risk_settings')||'{}');if(s.capital)document.getElementById('riskCapital').value=s.capital;if(s.riskPct)document.getElementById('riskPct').value=s.riskPct;if(s.maxPosPct)document.getElementById('maxPosPct').value=s.maxPosPct}catch(e){}}
...
.risk-planner{margin-bottom:16px}.risk-controls{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:14px 0}.risk-controls label{display:grid;gap:7px;color:var(--muted);font-size:12px;font-weight:800}.risk-controls input{width:100%;box-sizing:border-box;border:1px solid #294c70;background:#08182a;color:#f3f8ff;border-radius:10px;padding:11px 12px;font:inherit}.risk-controls input:focus{outline:2px solid rgba(76,195,255,.25);border-color:#4cc3ff}.position-plan{min-width:190px;line-height:1.55}.position-plan b{color:#dff7ff}.opening-history{margin-top:16px}.ok-text{color:#74f0b9;font-weight:900}@media(max-width:700px){.risk-controls{grid-template-columns:1fr}.risk-planner .category-head{align-items:flex-start}.position-plan{min-width:160px}}
'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/api/opening/history": return self.json({"ok": True, "items": opening_history_payload()})
