...
- 次日 `09:35,09:50,10:15,10:30` 自动核验，自动开盘确认只在 `09:35-11:30` 早盘窗口生成；
- 早盘页提供“盘中即时建仓评估”：交易日 `09:35-11:30`、`13:00-14:50` 可输入任意沪深 A 股代码，按日线计划、实时价格、分时均价线、开盘承接、回撤与止损给出“可小仓试仓 / 等待确认 / 不建议参与”等当前结论；非交易时段拒绝用旧行情模拟结论；
- 早盘页提供账户资金、单笔风险、单股仓位、组合总仓位和组合总止损风险约束，按候选排序自动折算整手组合预案；
- 保存并展示最近三日的开盘核验轨迹；每次定时核验可进入详情，逐股对比加入监控基准价、本次历史快照价和打开详情时重新获取的当前行情价及涨跌幅；可导出当前核验 CSV（已防范表格公式注入）；
- 交易复盘台：手动记录实际/模拟建仓、初始止损、移动保护价和退出结果，自动计算成本、风险、盈亏、收益率与 R 倍数；
- 早盘核验结果可一键带入交易复盘表单，形成“推荐—建仓—风控—退出—复盘”闭环；
- 个股基本面摘要与公开行情双数据源降级；
- 股票代码可打开**当日1分钟分时图**（成交均价、昨收基准、分钟成交量）以及日/周/月K线、MA5/10/20/60、历史缩放和平移；行情视图默认自动刷新（盘中约30秒、非盘中约5分钟），显示下次刷新倒计时，并支持鼠标/触屏拖拽和键盘平移；同时分开展示行情截止时间与本次查询时间。分时数据以东方财富为主、腾讯为备用，数据源不可用时会明确提示，不用旧数据冒充当日行情；
...
QUANT_WEB_INTRADAY_ENTRY_CACHE_TTL=30
