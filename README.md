...
- 工作日收盘后按 `15:35,21:00` 自动扫描，也可使用管理令牌手动补跑；运行状态会保留在服务端，重启后仍可查看最近一次触发、完成时间、数据质量和下一次工作日计划；
- 18 种战法，按趋势动量、突破、回踩低吸、K线形态、涨停情绪、超跌反转分类；
- 首页候选单选/多选/全选加入次日早盘监控，并支持任意六位沪深 A 股代码；
- 候选横向对比：勾选 2–4 只股票即可并排比较规则评分、RPS、趋势质量、盈亏比、计划区间、止损、量能、波动与风险标签；
- 单票风险预算：勾选 1 只候选后，按自填资金、单笔最大风险、单票资金上限、计划入场与止损价，离线计算理论整手数量、资金占用、理论最大亏损和目标 R 倍数；资金数字只在当前浏览器页面参与计算，不会提交到服务器；
...
- 数据质量闸门：行情日期不一致、非当日交易数据、K线失败率过高时，不会覆盖上一份有效候选或自动同步到早盘监控；晚间补跑失败也会自动恢复当日先前有效结果；
