...
import argparse, csv, datetime as dt, hmac, html, json, os, re, subprocess, sys, threading, time, traceback
...
def sync_auto_watchlist() -> Dict[str, Any]:
    ...
    if not signal:
        return {"synced": False, "reason": "尚无收盘扫描报告"}
    ...
    with WATCHLIST_LOCK:
        ...
        _write_watch_state_unlocked(state)
    _invalidate_opening_cache()
    return {"synced": True, "count": len(selected), "signal_file": signal.name}
...
def opening_session() -> Dict[str, Any]:
    ...
    if "09:30" <= clock < "11:30":
        return {"active": True, "mode": "morning", "label": "上午交易中", "time": n.strftime("%Y-%m-%d %H:%M:%S"), "next_action": "按价格确认条件核验"}
    if "11:30" <= clock < "13:00":
        return {"active": False, "mode": "noon", "label": "午间休市", "time": n.strftime("%Y-%m-%d %H:%M:%S"), "next_action": "13:00 开盘后可再次刷新"}
    if "13:00" <= clock < "15:00":
        return {"active": True, "mode": "afternoon", "label": "下午交易中", "time": n.strftime("%Y-%m-%d %H:%M:%S"), "next_action": "按价格确认条件核验"}
    return {"active": False, "mode": "after", "label": "已收盘", "time": n.strftime("%Y-%m-%d %H:%M:%S"), "next_action": "等待下一交易日开盘后的核验"}
...
def opening_decision(row: Dict[str, Any], quote: Dict[str, Any]) -> Dict[str, Any]:
    ...
    checks=[{"name":"计划买入区间","pass":in_zone,"text":f"现价 {price:.2f}；计划区间 {low:.2f}~{high:.2f}" if low is not None and high is not None else "没有有效买入区间"},{"name":"开盘溢价","pass":not too_high,"text":f"开盘较昨收 {gap:+.2f}%（阈值≤+4.5%）" if gap is not None else "开盘价不可用"},{"name":"止损保护","pass":not broken and stop is not None,"text":f"止损 {stop:.3f}" if stop is not None else "缺少明确止损"},{"name":"开盘承接","pass":bool(above),"text":"现价未明显跌破开盘价" if above else "现价弱于开盘价"},{"name":"昨日信号强度","pass":score_pass,"text":f"评分 {score:.1f}（阈值≥{score_limit}）"},{"name":"相对强度/风险","pass":rps_pass and risk_pass,"text":(f"RPS60 {rps60:.0f}" if rps60 is not None else "无RPS60")+("；无高波动/过热标签" if risk_pass else f"；风险：{risk}")}]
...
def opening_check_payload(force: bool = False) -> Dict[str, Any]:
    ...
    if not signal: return {"ok":False,"session":session,"message":"尚无收盘扫描信号，不能生成开盘核验。","rows":[],"summary":{},"schedule":OPENING_SCHEDULE,"run_state":public_run_state(OPENING_RUN_STATE),"watchlist":watch,"monitor_mode":mode}
    ...
    common={"source_date":source_date,"schedule":OPENING_SCHEDULE,"run_state":public_run_state(OPENING_RUN_STATE),"limit":OPENING_CHECK_LIMIT,"watchlist":watch,"monitor_mode":mode}
    ...
    if not force and cache.get("payload") and cache.get("source")==cache_key and now-safe_float(cache.get("at"))<OPENING_CACHE_TTL:
        result=dict(cache["payload"]); result["cached"]=True; return result
    def one(row):
        code=clean_stock_code(row.get("code"))
        try: return code,fetch_live_quote(code)
        except Exception as exc: return code,{"ok":False,"stale":False,"code":code,"name":row.get("name") or code,"source":"双源失败","error":f"{type(exc).__name__}: {exc}","fetched_at":now_cn().strftime("%Y-%m-%d %H:%M:%S")}
    quotes={}
    ...
    payload={"ok":True,"cached":False,"session":session,"message":"开盘确认采用胜率优先的保守多条件模型，基于前一交易日信号与公开行情快照；不会自动下单。","checked_at":now_cn().strftime("%Y-%m-%d %H:%M:%S"),"rows":out,"summary":summary,**common}
    with OPENING_LOCK: OPENING_CACHE.update({"at":now,"source":cache_key,"payload":payload})
    return payload
...
def recommendation_text(rows: List[Dict[str, Any]], market: str) -> str:
    ...
    return f"今日首选：{top.get('code')} {top.get('name')}｜{top.get('strategy_group')}｜{top.get('strategy')}｜评分 {top.get('score')}（{lvl}）。执行：{top.get('action')}；区间 {top.get('buy_zone')}；止损 {top.get('stop_loss')}{caution}。"

def load_latest_payload() -> Dict[str, Any]:
    signal=latest_signal_file()
    if not signal:
        return {"has_data": False, "message": "暂无报告，请点击“立即运行全市场扫描”。", "run_state": public_run_state(RUN_STATE), "reports": all_reports(), "feedback": feedback_overview(), "group_order": GROUP_ORDER, "strategy_book": STRATEGY_BOOK, "schedule": DEFAULT_SCHEDULE, "watchlist": watchlist_payload()}
    ...
    scope = {
        ...
        "spot_count": meta.get("spot_count"), "universe_count": meta.get("universe_count"), "kline_scanned_count": meta.get("kline_scanned_count"), "failed_count": meta.get("failed_count", len(meta.get("errors", []))), "execution_policy": meta.get("execution_policy", "候选信号需等待下一交易日价格条件确认，不构成直接交易指令。"),
    }
    return {"has_data": True, "date": f"{ymd[:4]}-{ymd[4:6]}-{ymd[6:]}" if len(ymd)==8 else ymd, "rows": rows, "top": rows[:12], "recommendation": recommendation_text(rows, market), "market": market, "group_counts": group_counts, "group_top": group_top, "group_order": GROUP_ORDER, "strategy_book": STRATEGY_BOOK, "scope": scope, "meta": meta, "signal_file": signal.name, "report_file": report.name if report else None, "report_excerpt": report_text[:12000], "run_state": public_run_state(RUN_STATE), "reports": all_reports(), "feedback": feedback_overview(), "schedule": DEFAULT_SCHEDULE, "watchlist": watchlist_payload()}
...
def run_scan_background(top:int=DEFAULT_TOP,max_stocks:int=DEFAULT_MAX_STOCKS,full:bool=DEFAULT_FULL,workers:int=DEFAULT_WORKERS)->bool:
    with RUN_LOCK:
        ...
        def target():
            log_path=LOG_DIR/f"scan_{now_cn().strftime('%Y%m%d_%H%M%S')}.log"; RUN_STATE["last_log"]=str(log_path)
            ...
            try:
                ...
                if p.returncode!=0:
                    ...
                    RUN_STATE["last_auto_sync"] = sync_auto_watchlist()
            except Exception as e:
...
def opening_page_html(payload: Dict[str, Any]) -> str:
    ...
    def money(v: Any) -> str: return "-" if v is None else f"{safe_float(v):.2f}"
    session = payload.get("session") or {}
    summary = payload.get("summary") or {}
    rows = payload.get("rows") or []
    watch = payload.get("watchlist") or {}
    watch_script = """<script>const openWatch=""" + script_json(watch) + r""";
const openEsc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
function openWatchHeaders(){const h={'Content-Type':'application/json'};const token=localStorage.getItem('quant_token')||'';if(token)h['X-Quant-Token']=token;return h}
async function openWatchSave(mode,codes){let r=await fetch('/api/watchlist',{method:'POST',headers:openWatchHeaders(),body:JSON.stringify({mode:mode,codes:codes})});if(r.status===401){const t=prompt('请输入管理令牌');if(!t)return;localStorage.setItem('quant_token',t);return openWatchSave(mode,codes)}const d=await r.json();if(!r.ok||!d.ok){alert(d.message||'更新失败');return}location.reload()}
...
(function(){const hero=document.querySelector('.opening-hero');if(!hero)return;const chips=(openWatch.items||[]).map(x=>`<span class="watch-chip"><b>${openEsc(x.code)}</b> ${openEsc(x.name)}<small>${openEsc(x.source)}</small><button onclick="removeWatch('${openEsc(x.code)}')">×</button></span>`).join('')||'<span class="muted">尚未选择；此时默认核验昨日高分候选。</span>';hero.insertAdjacentHTML('afterend',`<section class="card opening-watch" style="margin-bottom:16px"><div class="category-head"><div><h2>我的早盘建仓监控</h2><p>首页可勾选昨日推荐加入监控；也可在这里添加任意沪深 A 股六位代码。自选清单不为空时，本页仅核验清单内股票。</p></div><span class="pill">${openWatch.count||0}/${openWatch.max||100} 只</span></div><div class="watch-chips">${chips}</div><div class="watch-add"><input id="watchCode" class="fundamental-input" maxlength="80" placeholder="输入代码，如 000938；多个代码用逗号分隔"><button class="btn primary" onclick="addWatch()">加入监控</button><button class="btn" onclick="clearWatch()">清空手动监控</button><label class="monitor-auto"><input id="openAutoSync" type="checkbox" ${(openWatch.auto_sync||{}).enabled?'checked':''} onchange="setOpenAutoSync(this.checked)"> 自动同步收盘高分股</label></div><div class="small muted" style="margin-top:8px">开启后：每次收盘扫描成功，会自动同步前 ${(openWatch.auto_sync||{}).top_n||12} 只高分候选（纯涨停情绪类除外）；手动监控股票不会被覆盖。</div><div class="opening-note"><b>胜率优先的保守模型：</b>只有价格进入计划区间、未过度高开、未破止损、开盘承接正常，并同时通过评分、相对强度和风险门槛，才会显示“可计划内执行”。情绪类信号必须人工确认；自定义代码还须通过 MA20/MA60、均线斜率、量能与乖离门槛。</div></section>`);document.getElementById('watchCode').addEventListener('keydown',e=>{if(e.key==='Enter')addWatch()})})();</script>"""
    ...
    for r in rows:
        ...
        row_html.append(f'<tr><td><b>{esc(r.get("code"))}</b><br><span class="muted small">{esc(r.get("name"))} · {esc(r.get("source"))}</span></td><td><span class="opening-badge {cls}">{esc(r.get("status"))}</span><br><span class="muted small">评分 {safe_float(r.get("score")):.1f}｜{esc(r.get("level"))}</span></td><td>{esc(r.get("strategy_group"))}<br><span class="small muted">{esc(r.get("strategy"))}</span></td><td>{money(q.get("price"))}<br><span class="small muted">涨跌 {pct_text}｜{esc(q.get("source"))}<br>{esc(q.get("trade_time"))}</span></td><td>开 {money(q.get("open"))}<br><span class="small muted">高开 {gap_text}</span></td><td>{esc(r.get("buy_zone"))}<br><span class="small muted">止损 {money(r.get("stop_loss"))}</span></td><td class="opening-checks">{check_html or "-"}</td><td class="opening-reason">{esc(r.get("reason"))}<br><span class="small muted">原计划：{esc(r.get("action"))}</span></td></tr>')
...
function dailyOverview(d,rows,meta,scope,core,avgRps){const market=d.market||'未识别';const tone=(market.includes('弱')||market.includes('防守'))?'weak':(market.includes('强')?'strong':'');const stages={};rows.forEach(r=>{const x=r.execution_stage||'待确认';stages[x]=(stages[x]||0)+1});const stageRows=Object.entries(stages).sort((a,b)=>b[1]-a[1]);const maxStage=Math.max(1,...stageRows.map(x=>x[1]));const stageHtml=stageRows.map(([name,count])=>`<div class="dist-row"><span>${esc(name)}</span><div class="dist-bar"><i style="width:${Math.round(count/maxStage*100)}%"></i></div><b>${count}</b></div>`).join('');const top=rows[0]||{};const last=(d.run_state||{}).last_finished||'尚未完成';return `<section class="card overview-panel" id="overview"><div class="overview-head"><div><span class="market-badge ${tone}">市场状态 · ${esc(market)}</span><h2 style="margin-top:11px">今日量化决策摘要</h2><div class="recommend">${esc(d.recommendation)}<div class="small muted" style="margin-top:7px">排序第一仅表示规则评分靠前，仍需遵守“执行层级、价格确认、止损”三个条件。</div></div></div><div class="scan-chip"><b>自动扫描</b><br>每日 ${esc(d.schedule||'15:35,21:00')}<br>最近完成：${esc(last)}<br>数据日：${esc(d.date)}</div></div><div class="kpis">${kpi('全市场行情',scope.spot_count,'进入行情列表')}${kpi('基础过滤池',scope.universe_count,'过滤ST/流动性等')}${kpi('K线深筛',scope.kline_scanned_count,'逐只匹配战法')}${kpi('候选信号',rows.length,'不是全部都要买')}${kpi('核心/优先',core,'优先复核对象')}${kpi('平均RPS',isFinite(avgRps)?avgRps:'-','候选横向强度')}</div><div class="overview-grid"><div class="overview-box"><h3>执行政策与筛选范围</h3><div class="small">${esc(scope.execution_policy||'候选需等待价格确认。')}</div><div class="small muted" style="margin-top:7px">${esc(scope.text||'')}</div><div class="quick-nav"><a href="#candidates">直接看候选</a><a href="#fundamentals">查询基本面</a><a href="#strategies">查看战法</a><a href="#feedback">查看复盘</a></div></div><div class="overview-box"><h3>执行层级分布</h3><div class="dist-list">${stageHtml||'<span class="muted small">暂无候选</span>'}</div></div></div></section>`}
...
function render(){const d=window.__DATA__, el=document.getElementById('app'); if(!d.has_data){el.innerHTML=`<section class="hero"><div class="title"><span class="eyebrow">A股量化驾驶舱</span><h1>暂无扫描结果</h1><p>${esc(d.message||'')}</p></div><div class="actions"><button class="btn primary" onclick="runScan()">立即运行全市场扫描</button></div></section>${statusHtml(d.run_state)}`;return;}
 ...
 const filteredBase=rows.filter(rowVisible); const filtered=priorityOnly?filteredBase.filter(r=>['核心推荐','优先关注'].includes(r.level)):filteredBase; const displayRows=showAllRows?filtered:filtered.slice(0,20); const leaders=Object.entries(d.group_top||{}).map(([g,r])=>`<div class="leader-card"><div class="muted small">${esc(g)}</div><b>${esc(r.code)} ${esc(r.name)}</b><div class="small">${esc(r.strategy)}｜评分 ${esc(r.score)}｜RPS ${esc(r.rps_combo||'-')}</div><div class="bar"><i style="width:${Math.min(100,parseFloat(r.score)||0)}%"></i></div></div>`).join('');
 ...
 const autoCfg=(d.watchlist||{}).auto_sync||{}; const quickNav=`<nav class="quick-nav"><div class="quick-links"><a href="#overview">今日总览</a><a href="#fundamentals">个股基本面</a><a href="#strategies">战法导航</a><a href="#feedback">历史反馈</a><a href="#candidates">候选列表</a><a href="/opening">早盘确认</a></div><div class="quick-state"><span>信号日 ${esc(d.date||'-')}</span><span class="${autoCfg.enabled?'on':'off'}">${autoCfg.enabled?'自动监控已开启':'自动监控未开启'}</span><span>监控 ${(d.watchlist||{}).count||0} 只</span></div></nav>`;
...
function showReport(){document.getElementById('reportText').textContent=window.__DATA__.report_excerpt||'';document.getElementById('modal').classList.add('show')}function hideReport(){document.getElementById('modal').classList.remove('show')}render();setupMonitorObserver();
'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/opening": return self.send_body(opening_page_html(opening_check_payload()).encode())
        if path=="/api/fundamentals":
            ...
            try:
                ...
                traceback.print_exc()
                return self.json({"ok": False, "message": f"基本面数据暂不可用：{e}"}, 503)
        if path=="/healthz": return self.json({"ok":True,"service":"quant-web","time":now_cn().strftime("%Y-%m-%d %H:%M:%S"),"scan_running":bool(RUN_STATE.get("running"))})
        ...
        if path=="/api/watchlist": return self.json({"ok": True, "watchlist": watchlist_payload()})
    ...
    def do_POST(self):
        ...
        if u.path=="/api/run":
            ...
            return self.json({"ok":ok,"message":"已开始后台全市场扫描，完成后刷新查看。" if ok else "已有扫描正在运行。","state":RUN_STATE})
        return self.send_error(404,"not found")
