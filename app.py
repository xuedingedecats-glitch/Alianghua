...
def kline_payload(raw_code: Any, raw_period: Any = "day", raw_limit: Any = 120) -> Dict[str, Any]:
    ...
    limit = max(40, min(limit, 640)); key=f"{code}:{period}:{limit}"; now=time.time()
...
let klineState={code:'',name:'',period:'day',data:null,mas:new Set([5,10,20,60]),request:0,limit:240,visible:240,offset:0};
...
function closeKline(){klineState.request++;document.body.classList.remove('kline-open');const modal=document.getElementById('klineModal');modal.classList.remove('show','fullscreen');document.getElementById('klineFullscreenBtn').textContent='全屏'}
