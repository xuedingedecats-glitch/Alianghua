...
def opening_check_payload(force: bool = False) -> Dict[str, Any]:
    ...
    if not signal: return {"ok":False,"session":session,"message":"尚无收盘扫描信号，不能生成开盘核验。","rows":[],"summary":{},"schedule":OPENING_SCHEDULE,"run_state":public_run_state(OPENING_RUN_STATE, OPENING_LOCK),"watchlist":watch,"monitor_mode":mode}
    ...
    common={"source_date":source_date,"schedule":OPENING_SCHEDULE,"run_state":public_run_state(OPENING_RUN_STATE, OPENING_LOCK),"limit":OPENING_CHECK_LIMIT,"watchlist":watch,"monitor_mode":mode}
