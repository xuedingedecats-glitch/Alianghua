...
let klineState={code:'',name:'',period:'day',data:null,mas:new Set([5,10,20,60]),request:0,limit:240,visible:240,offset:0,auto:true,timer:null};
function openKline(code){const clean=String(code||'').replace(/\D/g,'').slice(0,6),row=(window.__DATA__.rows||[]).find(x=>String(x.code)===clean);klineState.code=clean;klineState.name=row?.name||'';klineState.period='day';klineState.data=null;klineState.offset=0;klineState.visible=klineState.limit;document.body.classList.add('kline-open');document.getElementById('klineModal').classList.add('show');document.getElementById('klineTitle').textContent=`${klineState.code} ${klineState.name} K线图`;document.querySelectorAll('[data-period]').forEach(b=>b.classList.toggle('active',b.dataset.period==='day'));loadKline()}
function closeKline(){klineState.request++;clearTimeout(klineState.timer);klineState.timer=null;document.body.classList.remove('kline-open');const modal=document.getElementById('klineModal');modal.classList.remove('show','fullscreen');document.getElementById('klineFullscreenBtn').textContent='全屏'}
...
function scheduleKlineRefresh(seconds){clearTimeout(klineState.timer);klineState.timer=null;if(!klineState.auto||!document.getElementById('klineModal')?.classList.contains('show'))return;const wait=Math.max(15,Number(seconds)||300);klineState.timer=setTimeout(()=>loadKline(true),wait*1000)}
function setKlineAutoRefresh(on){klineState.auto=!!on;if(on)scheduleKlineRefresh(klineState.data?.auto_refresh_seconds||300);else{clearTimeout(klineState.timer);klineState.timer=null}}
