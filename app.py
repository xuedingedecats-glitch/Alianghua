...
/* 候选对比与风险预算：仅在浏览器本地计算，不会提交账户资金或下单。 */
body.decision-open{overflow:hidden}.decision-modal{padding:18px;overscroll-behavior:contain}.decision-card{position:relative;width:min(1180px,calc(100vw - 36px));max-height:calc(100vh - 36px);overflow:auto;padding:24px}.decision-head{padding-right:38px}.decision-head h2{margin:8px 0 4px}.decision-head p{margin:0;line-height:1.65}.compare-grid{display:grid;grid-template-columns:repeat(var(--compare-cols,2),minmax(220px,1fr));gap:12px;margin-top:16px}.compare-stock{border:1px solid #2d567b;background:linear-gradient(160deg,#0f2742,#081829);border-radius:17px;padding:14px;min-width:0}.compare-stock h3{margin:0 0 5px;font-size:18px}.compare-stock .compare-code{color:#80dfff;font-size:12px;font-weight:900;letter-spacing:.6px}.compare-metrics{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:12px 0}.compare-metric{border:1px solid #274866;background:#091a2b;border-radius:11px;padding:8px}.compare-metric span{display:block;color:#8da8c5;font-size:11px;margin-bottom:4px}.compare-metric b{font-size:15px;color:#e7f3ff;word-break:break-word}.compare-row{border-top:1px dashed #2d4b68;padding:8px 0;display:grid;grid-template-columns:78px 1fr;gap:8px;font-size:12px;line-height:1.5}.compare-row span{color:#94afca}.compare-row b{color:#e1efff;font-weight:800;word-break:break-word}.compare-risk{display:inline-block;border:1px solid #77513f;background:#3b241d;color:#ffd4a0;border-radius:999px;padding:3px 7px;font-size:11px;margin-top:8px}.compare-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.compare-actions .filter-btn{font-size:12px}.compare-empty,.budget-empty{margin-top:14px;padding:18px;border:1px dashed #355d7f;border-radius:14px;color:#9db9d6;background:#09192a}.budget-layout{display:grid;grid-template-columns:minmax(260px,.92fr) minmax(340px,1.08fr);gap:14px;margin-top:16px}.budget-form{display:grid;gap:10px}.budget-form label{display:grid;gap:6px;color:#b9d4ee;font-size:12px;font-weight:800}.budget-form input{width:100%;border:1px solid #355a7c;border-radius:10px;background:#081728;color:#eaf5ff;padding:10px;font:inherit}.budget-form input:focus{outline:2px solid #2faee7;border-color:#6ad9ff}.budget-form .small{line-height:1.55}.budget-result{border:1px solid #2c6c78;background:linear-gradient(145deg,#0a2736,#081827);border-radius:16px;padding:15px}.budget-result h3{margin:0 0 10px}.budget-result-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.budget-result-item{padding:10px;border:1px solid #2b5368;background:#081b2c;border-radius:11px}.budget-result-item span{display:block;color:#8badc8;font-size:11px}.budget-result-item b{display:block;font-size:18px;margin-top:4px}.budget-warning{margin-top:10px;padding:10px;border:1px solid #725a35;background:#2c2417;border-radius:10px;color:#ffe3a5;font-size:12px;line-height:1.6}.budget-ok{margin-top:10px;padding:10px;border:1px solid #28664f;background:#102c25;border-radius:10px;color:#a9f0d1;font-size:12px;line-height:1.6}.compare-action{border-color:#4c83ff!important;background:#193a76!important;color:#dceaff!important}.budget-action{border-color:#3c9675!important;background:#10533f!important;color:#dfffee!important}@media(max-width:760px){.decision-modal{padding:0}.decision-card{width:100vw;max-height:100vh;min-height:100vh;border-radius:0;border:0;padding:18px}.compare-grid{grid-template-columns:1fr!important}.budget-layout{grid-template-columns:1fr}.budget-result-grid{grid-template-columns:1fr 1fr}.compare-row{grid-template-columns:72px 1fr}}
...
function selectedCandidateRows(){const selected=monitorDraft instanceof Set?[...monitorDraft]:[];return (window.__DATA__.rows||[]).filter(r=>selected.includes(String(r.code||'')))}
function candidateNum(value,digits=2){const n=Number(value);return Number.isFinite(n)?n.toFixed(digits):'-'}
function candidateText(value){return value===undefined||value===null||value===''?'-':esc(value)}
function candidateEntryPrice(row){const values=String(row.buy_zone||'').match(/\d+(?:\.\d+)?/g)||[];const nums=values.map(Number).filter(Number.isFinite);if(nums.length>=2)return (nums[0]+nums[1])/2;if(nums.length===1)return nums[0];const close=Number(row.close);return Number.isFinite(close)&&close>0?close:0}
function closeCandidateCompare(){const modal=document.getElementById('compareModal');if(modal)modal.classList.remove('show');document.body.classList.remove('decision-open')}
function closeRiskBudget(){const modal=document.getElementById('budgetModal');if(modal)modal.classList.remove('show');document.body.classList.remove('decision-open')}
function closeDecisionOnBackdrop(event,id){if(!event||event.target?.id!==id)return;if(id==='compareModal')closeCandidateCompare();else closeRiskBudget()}
document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;const compare=document.getElementById('compareModal'),budget=document.getElementById('budgetModal');if(budget?.classList.contains('show'))closeRiskBudget();else if(compare?.classList.contains('show'))closeCandidateCompare()});
function candidateCard(row){const code=String(row.code||'');const risk=String(row.risk_tags||'常规');return `<article class="compare-stock"><div class="compare-code">${candidateText(code)} · ${candidateText(row.level||'待复核')}</div><h3>${candidateText(row.name||code)}</h3><div class="small muted">${candidateText(row.strategy_group||'综合类')}</div><div class="compare-metrics"><div class="compare-metric"><span>规则评分</span><b>${candidateNum(row.score,1)}</b></div><div class="compare-metric"><span>综合 RPS</span><b>${candidateNum(row.rps_combo,1)}</b></div><div class="compare-metric"><span>趋势质量</span><b>${candidateNum(row.trend_quality,1)}</b></div><div class="compare-metric"><span>盈亏比</span><b>${candidateNum(row.risk_reward,2)} R</b></div></div><div class="compare-row"><span>买入区间</span><b>${candidateText(row.buy_zone)}</b></div><div class="compare-row"><span>止损 / 目标</span><b>${candidateNum(row.stop_loss,3)} / ${candidateNum(row.target,3)}</b></div><div class="compare-row"><span>量能 / 波动</span><b>${candidateNum(row.vol_ratio20,2)} 倍 / ATR ${candidateNum(row.atr_pct,2)}%</b></div><div class="compare-row"><span>执行条件</span><b>${candidateText(row.action)}</b></div><span class="compare-risk">风险：${candidateText(risk)}</span><div class="compare-actions"><button type="button" class="filter-btn" onclick="openKline('${code}')">查看 K 线</button><button type="button" class="filter-btn budget-action" onclick="openRiskBudget('${code}')">风险预算</button></div></article>`}
function openCandidateCompare(){const selected=selectedCandidateRows();if(selected.length<2){alert('请先在候选表中勾选至少 2 只股票再比较');return}if(selected.length>4){alert('为保证横向阅读清晰，请一次勾选 2–4 只股票');return}const content=document.getElementById('compareContent');if(!content)return;content.innerHTML=`<div class="small muted">已选 ${selected.length} 只。评分与 RPS 用于横向复核；最终是否执行仍以开盘确认、价格条件和个人风险承受能力为准。</div><div class="compare-grid" style="--compare-cols:${selected.length}">${selected.map(candidateCard).join('')}</div>`;document.getElementById('compareModal').classList.add('show');document.body.classList.add('decision-open')}
function riskBudgetHtml(row){const code=String(row.code||'');const entry=candidateEntryPrice(row),stop=Number(row.stop_loss),target=Number(row.target);return `<div class="budget-layout"><form id="riskBudgetForm" class="budget-form"><div class="watch-chip"><b>${candidateText(code)} ${candidateText(row.name||'')}</b><small>${candidateText(row.buy_zone||'计划区间缺失')}｜止损 ${candidateNum(stop,3)}</small></div><label>可用于该策略的总资金（元）<input id="budgetCapital" type="number" min="0" step="1000" value="100000" inputmode="decimal" required></label><label>单笔最大账户风险（%）<input id="budgetRiskPct" type="number" min="0.1" max="5" step="0.1" value="0.5" inputmode="decimal" required></label><label>单票最大资金占比（%）<input id="budgetExposurePct" type="number" min="1" max="100" step="1" value="20" inputmode="decimal" required></label><label>计划入场价（元）<input id="budgetEntry" type="number" min="0.001" step="0.001" value="${entry||''}" inputmode="decimal" required></label><label>计划止损价（元）<input id="budgetStop" type="number" min="0.001" step="0.001" value="${Number.isFinite(stop)&&stop>0?stop:''}" inputmode="decimal" required></label><input id="budgetTarget" type="hidden" value="${Number.isFinite(target)&&target>0?target:''}"><button type="submit" class="btn primary">计算理论仓位</button><div class="small muted">默认以 100 股为一手向下取整，并同时受“单笔风险”和“单票资金占比”双重限制。</div></form><section id="budgetResult" class="budget-result"><h3>等待输入</h3><p class="muted small">填写后计算。工具只做风险金额和数量的算术演算，不保证成交或收益，也不替代你对流动性、消息面与开盘条件的判断。</p></section></div>`}
function renderRiskBudget(){const capital=Number(document.getElementById('budgetCapital')?.value),riskPct=Number(document.getElementById('budgetRiskPct')?.value),exposurePct=Number(document.getElementById('budgetExposurePct')?.value),entry=Number(document.getElementById('budgetEntry')?.value),stop=Number(document.getElementById('budgetStop')?.value),target=Number(document.getElementById('budgetTarget')?.value),box=document.getElementById('budgetResult');if(!box)return;if(![capital,riskPct,exposurePct,entry,stop].every(Number.isFinite)||capital<=0||riskPct<=0||exposurePct<=0||entry<=0||stop<=0){box.innerHTML='<div class="budget-warning">请填写有效的资金、比例、入场价与止损价。</div>';return}if(stop>=entry){box.innerHTML='<div class="budget-warning">止损价必须低于计划入场价；请先确认价格计划，再计算仓位。</div>';return}const unitRisk=entry-stop,maxLoss=capital*riskPct/100,maxExposure=capital*exposurePct/100,byRisk=Math.floor(maxLoss/unitRisk/100)*100,byExposure=Math.floor(maxExposure/entry/100)*100,shares=Math.max(0,Math.min(byRisk,byExposure)),cost=shares*entry,theoreticalLoss=shares*unitRisk,potential=Number.isFinite(target)&&target>entry?shares*(target-entry):null,limit=byRisk<=byExposure?'单笔风险上限':'单票资金占比上限';const warning=shares<100?'<div class="budget-warning">在当前风险约束下，理论数量不足 100 股。不要为了凑整而放宽止损或提高风险比例。</div>':`<div class="budget-ok">当前受 <b>${limit}</b> 约束。实际委托前仍需考虑手续费、滑点、涨跌停、最小交易单位、流动性及账户可用资金。</div>`;box.innerHTML=`<h3>理论仓位结果</h3><div class="budget-result-grid"><div class="budget-result-item"><span>建议上限股数</span><b>${shares.toLocaleString()} 股</b></div><div class="budget-result-item"><span>预计占用资金</span><b>¥${cost.toLocaleString(undefined,{maximumFractionDigits:0})}</b></div><div class="budget-result-item"><span>理论最大亏损</span><b>¥${theoreticalLoss.toLocaleString(undefined,{maximumFractionDigits:0})}</b></div><div class="budget-result-item"><span>单股风险 / 风险比</span><b>¥${unitRisk.toFixed(3)} / ${((unitRisk/entry)*100).toFixed(2)}%</b></div>${potential!==null?`<div class="budget-result-item"><span>到目标的理论收益</span><b>¥${potential.toLocaleString(undefined,{maximumFractionDigits:0})}</b></div><div class="budget-result-item"><span>目标盈亏比</span><b>${((target-entry)/unitRisk).toFixed(2)} R</b></div>`:''}</div>${warning}`}
function openRiskBudget(code){let selected=selectedCandidateRows();if(code)selected=(window.__DATA__.rows||[]).filter(r=>String(r.code||'')===String(code));if(selected.length!==1){alert('请先勾选 1 只股票后使用风险预算；横向对比需勾选 2–4 只。');return}const content=document.getElementById('budgetContent');if(!content)return;content.innerHTML=riskBudgetHtml(selected[0]);content.querySelector('#riskBudgetForm').onsubmit=e=>{e.preventDefault();renderRiskBudget()};document.getElementById('budgetModal').classList.add('show');document.body.classList.add('decision-open');renderRiskBudget()}
