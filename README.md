...
- 次日 `09:35,09:50,10:15,10:30` 做保守多条件确认，不自动下单；
- 早盘页提供账户资金、单笔风险和单股仓位上限计算，自动折算为整手仓位预案；
...
- 请求体限制、接口限速、并发上限、安全响应头和 systemd 沙箱。
...
- 不要把 `/etc/quant-web.env`、管理令牌、`opening_watchlist.json`、报告、缓存、日志或虚拟环境提交到 GitHub；
